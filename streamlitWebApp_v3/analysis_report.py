...
def load_timestamp_results(results_path, log_path):
    """특정 타임스탬프의 JSON 결과 파일들을 로드"""
    
    if not os.path.exists(results_path):
        return None, f"결과 폴더를 찾을 수 없습니다: {results_path}"
    
    json_files = glob.glob(os.path.join(results_path, "*.json"))
    if not json_files:
        error_msg = f"JSON 결과 파일을 찾을 수 없습니다: {results_path}"
...
def create_execution_timeline(log_path, timestamp):
...
def calculate_execution_time(log_path):
    """로그에서 실행 시간을 계산"""
...
def download_log_file(log_path, timestamp):
    """로그 파일 다운로드 함수"""
...
def main(results_path, log_path, timestamp, is_guest_view=False):
    """메인 분석 리포트 페이지"""
    ...
    if not is_guest_view:
        st.markdown("---")
        col1, col2, col3 = st.columns([1, 1, 2]) # st.columns를 한 번만 사용
        
        with col1:
            # log_path와 timestamp를 모두 전달하도록 수정
            download_log_file(log_path, timestamp)
    ...
    with st.spinner("📂 분석 결과 데이터 로딩 중..."):
        result_data, error = load_timestamp_results(results_path, log_path)
        ...
        if error:
            if isinstance(error, dict):
                ...
                if error.get('has_log'):
                    ...
                    with col1:
                        st.info(f"📄 로그 파일: `{error['log_path']}`") 
                    with col2:
    ...
    with tab4:
        ...
        # 실행 타임라인
        fig_timeline = create_execution_timeline(log_path, timestamp)
        if fig_timeline:
        ...
        with col1:
            # 실제 실행 시간 계산
            execution_time = calculate_execution_time(log_path)
            if execution_time:
    ...
    with tab5:
        ...
        if os.path.exists(log_path):
            ...
            st.warning(f"로그 파일을 찾을 수 없습니다: {log_path}")
