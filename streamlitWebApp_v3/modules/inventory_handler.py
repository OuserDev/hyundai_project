        ...
        for part in parts[1:]:
            if '=' in part:
                ...
                if key == 'ansible_host':
                    ...
                    # 1. value 양쪽의 따옴표를 먼저 제거합니다. (핵심 수정사항)
                    # 예: '"PC-Linux,Server-Linux"' -> 'PC-Linux,Server-Linux'
                    cleaned_value = value.strip('"\'') 
                    
                    # 2. 따옴표가 제거된 값을 쉼표로 분리합니다.
                    # .title()은 'PC-Linux'를 'Pc-Linux'로 변경하여 문제를 일으킬 수 있으므로 제거합니다.
                    services = [s.strip() for s in cleaned_value.split(',')]
