            ...
            try:
                ...
                if timestamp_match:
                    ...
                    record = {
                        ...
                        'status': '완료' if has_results else '실행만'
                    }
...
def render_sidebar_with_history():
    """확장된 사이드바 렌더링 (개선된 버전)"""
    ...
    # 기존 기록 디버깅 버튼 (개발 시에만)
    if st.sidebar.button("🔍 기존 기록 스캔 (디버그)"):
        debug_existing_logs()
    ...
    if analysis_history:
        ...
        for i, record in enumerate(analysis_history):
            ...
            # 상태 아이콘 결정
            status_icon = "✅" if record['has_results'] else "⏸️"
